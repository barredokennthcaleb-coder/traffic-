...
<?= $this->section('title') ?>Violation Details - <?= esc($violation['ticket_id'] ?? 'TCT Ticket') ?><?= $this->endSection() ?>
...
<?php
    ...
    if (!function_exists('isTctCheckedUser')) {
        function isTctCheckedUser($stdName, $activeNames) {
...
<div class="container-fluid py-3 no-print">
    <div class="d-flex justify-content-between align-items-center">
        <a href="<?= base_url('user/violations') ?>" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-arrow-left me-1"></i> Back to Violations
        </a>
        <div class="d-flex gap-2">
            <?php if (($violation['status'] ?? '') === 'Pending'): ?>
                <a href="<?= base_url('user/pay/' . $violation['ticket_id']) ?>" class="btn btn-success btn-sm shadow-sm">
                    <i class="bi bi-credit-card me-1"></i> Pay Violation
                </a>
            <?php endif; ?>
        ...
        <div class="tct-doc-section">
            ...
            <div class="tct-checklist-container">
                <?php foreach ($standardViolations as $std): 
                    $checked = isTctCheckedUser($std, $activeViolationNames);
                ?>
